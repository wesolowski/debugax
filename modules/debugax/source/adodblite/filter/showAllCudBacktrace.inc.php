...
function& adodb_log_sql(&$conn,$sql,$inputarr)
{
    ...
    */
    ...
    if (!empty($conn->_logsql) && $sIdent != false && strpos(strtolower(trim($sql)), oxSession::getVar( 'debugPHPSearch' )) !== false
        && (strpos(strtolower(trim($sql)), 'select') !== true) && (strpos(strtolower(trim($sql)), 'show') !== true)) {
